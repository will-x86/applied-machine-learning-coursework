...
def train_and_eval(
    ...
    print(f"\n[{name}] spam removed: {(val_spam==-1).sum()} | accuracy: {acc:.3f}")
    print(cm)
    return cm
...
def run_task1():
    ...
    for ax, cm, title in zip(axes, [cm_regex, cm_iso], ["Regex", "IsolationForest"]):
